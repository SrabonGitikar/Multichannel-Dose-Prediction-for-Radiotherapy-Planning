...
class PhysicsGuidedDoseLoss(nn.Module):
    ...
    def forward(self, pred_dose, true_dose, bladder_mask, rectum_mask,
        ...
        if ptv_n > 0:
            # L2 penalty provides gradient scaling: extreme outliers
        ...
        # Restrict to BODY voxels only — air voxels dilute .mean() by ~9×.
...
def main():
    ...
    # All physics lambdas are explicitly set to 0.0 for this baseline script.
    # Only lambda_mse is active.
    ...
    print(f"\n{'='*60}")
    print(f"BASELINE MODEL: All physics lambdas are ZERO for all epochs")
    print(f"lambda_mse = 25.0  (constant throughout)")
    print(f"{'='*60}")
    logger.info("Baseline model started: lambda_mse=25.0, all physics constraints=0.0")
    ...
    for epoch in range(epochs):
        ...
        for attr, target in PHYSICS_TARGET_LAMBDAS.items():
            setattr(loss_function, attr, target)

        phase_tag = "BASELINE (MSE ONLY)"

        print(f"\nEpoch {epoch + 1}/{epochs}  [{phase_tag}]  (lr={current_lr:.2e})")
        logger.info(f"Epoch {epoch + 1}/{epochs} [{phase_tag}] lr={current_lr:.2e}")
