    ...
    def forward(self, pred_dose, true_dose, bladder_mask, rectum_mask,
        ...
        for name, data in sib_targets.items():
            ...
            if n_mask > 0:
                ...
                # The Floor: Strict L1 penalty for any underdosing below Rx
                underdose_penalty = torch.relu(rx_norm - target_voxels)
                loss_ptv += underdose_penalty.sum() / n_mask

                # The Ceiling: Strict L1 penalty for any overdosing above ICRU limit
                overdose_penalty = torch.relu(target_voxels - ceil_norm)
